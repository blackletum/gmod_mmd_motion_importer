...
MMDVMDNPC.I18N.en = {
    ...
    ["mmd_vmd_npc.ui.radial_wheel_help"] = "The wheel is bound to K by default (change it with the binder above, or in console: bind <key> +mmd_wheel). Hold the key to open the wheel, point at a motion, then release or click to play it on yourself — it builds automatically first if your playermodel has no cache yet. The top slot is always Stop; scroll the mouse wheel to switch pages. The dropdown above the wheel picks what it shows: All Categories or a single category lists every dance automatically, while ★ Favorites is your own curated list (add dances via the Motion Manager or the button above; right-click a slot there to remove it, which also works for leftovers whose motion file was deleted).",
    ...
    ["mmd_vmd_npc.ui.wheel_bind_help"] = "Hold this key to open the motion wheel (default K). Changing it rebinds the key immediately.",
...
MMDVMDNPC.I18N.zh = merge_i18n({
    ...
    ["mmd_vmd_npc.ui.wheel_bind_help"] = "按住该键打开动作转盘（默认 K）。修改后立即重新绑定。",
...
MMDVMDNPC.I18N.ja = merge_i18n({
    ...
    ["mmd_vmd_npc.ui.wheel_bind_help"] = "このキーを押している間ホイールが開きます（既定 K）。変更すると即座に再バインドされます。",
